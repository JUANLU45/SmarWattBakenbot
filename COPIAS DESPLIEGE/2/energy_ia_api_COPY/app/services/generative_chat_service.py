...
class EnterpriseGenerativeChatService:
    ...
    def _consult_expert_bot(
        ...
        try:
            ...
            # SOLUCIÓN ANTI-DEADLOCK: LLAMADA DIRECTA INTERNA
            # En lugar de HTTP call al mismo servicio, usar lógica interna directa
            from app.routes import get_recommender_service
            ...
            # LLAMADA DIRECTA - NO HTTP - ELIMINA DEADLOCK
            service = get_recommender_service()
            
            # Para recomendaciones necesitamos perfil de consumo
            # Simulamos un perfil básico o usamos datos por defecto
            basic_profile = {
                "monthly_consumption": 300,  # kWh promedio
                "peak_usage_hours": [19, 20, 21],  # Horas punta típicas
                "user_type": "residential"
            }
            
            # Obtener recomendaciones usando lógica interna directa
            recommendations = service.get_advanced_recommendation(basic_profile)
            
            logging.info("✅ Recomendaciones de tarifas obtenidas exitosamente - LLAMADA DIRECTA")
    ...
    def _get_current_market_prices(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Formatear datos igual que en el endpoint original
            market_data = {
                "tariffs": tariffs,
                "stats": {
            ...
            logging.info("✅ Datos de precios de mercado obtenidos exitosamente - LLAMADA DIRECTA")
            return market_data
