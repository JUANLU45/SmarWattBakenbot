...
def _get_esios_api_key() -> str:
    """🔑 TOKEN CORRECTO DESDE SECRET MANAGER EN CLOUD FUNCTIONS"""

    # En Cloud Functions, leer desde Secret Manager
    if CLOUD_AVAILABLE:
        try:
            project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "smatwatt")
            client = secretmanager.SecretManagerServiceClient()
            secret_name = f"projects/{project_id}/secrets/ESIOS_API_KEY/versions/latest"
            ...
            logger.warning(f"⚠️ No se pudo leer desde Secret Manager: {e}")

    # Fallback: token correcto hardcodeado
    return "ac5c7863608aae858a07e6e31cc9497275ecbe4a5d57f6fa769abd9bee350c94"
...
class ESIOSConfig:
    ...
    api_key: str = field(default_factory=lambda: _get_esios_api_key())
...
class ESIOSTariffUpdater:
    ...
    def __init__(self, config: ESIOSConfig):
        ...
        self.session.headers.update(
            {
                ...
                "x-api-key": self.config.api_key,  # FORMATO CORRECTO ESIOS: x-api-key sin comillas
        ...
        # DEBUG: Log de token para verificar
        token_preview = (
            f"{self.config.api_key[:10]}...{self.config.api_key[-4:]}"
