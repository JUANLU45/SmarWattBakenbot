...
class EnterpriseGenerativeChatService:
    ...
    def _consult_expert_bot(
        ...
        """Consulta datos del mercado de tarifas para precios actuales - LLAMADA INTERNA DIRECTA"""
        try:
            # SOLUCIÓN DEFINITIVA: Llamada interna directa sin HTTP
            ...
            if tariffs:
                # Estadísticas del mercado (idénticas al endpoint)
                stats = {
                    ...
                    "pvpc_tariffs": len(
                        [t for t in tariffs if t.get("is_pvpc", False)]
                    ),
                    "active_tariffs": len(
                        [t for t in tariffs if t.get("is_active", False)]
                    ),
                    "last_updated": max(
                ...
                logging.info(
                    f"✅ Datos del mercado obtenidos directamente: {len(tariffs)} tarifas"
                )
                return {"tariffs": tariffs, "market_statistics": stats}
            else:
                logging.warning("⚠️ No se obtuvieron tarifas del mercado")
                return {}
