...
class EnterpriseVertexAIService:
    ...
    def get_enterprise_tariff_recommendation(
        ...
        try:
            ...
            # 🌦️ DATOS METEOROLÓGICOS (SIEMPRE - INCLUSO SIN VERTEX AI)
            weather_data = self._get_weather_data_for_user(user_profile)
            ...
            if current_app.config.get("VERTEX_AI_ENABLED"):
                ...
                logger.info(
                    "⚠️ Vertex AI está desactivado. Usando algoritmos matemáticos con datos meteorológicos."
                )
                # Creamos análisis matemático con datos meteorológicos REALES
                ml_user_insights = self._create_mathematical_analysis_with_weather(
                    user_profile, weather_data
                )
    ...
    def _create_mathematical_analysis_with_weather(
        self, user_profile: Dict, weather_data: Dict
    ) -> Dict[str, Any]:
        """🧮 Crea análisis matemático robusto con datos meteorológicos SIN Vertex AI"""
        try:
            logger.info("🧮 Iniciando análisis matemático con datos meteorológicos")

            # 📊 ANÁLISIS DE CONSUMO CON CLIMA
            consumption_analysis = self._analyze_consumption_with_weather(
                user_profile, weather_data
            )

            # 🏠 ANÁLISIS DE EFICIENCIA PERSONALIZADO
            efficiency_analysis = self._analyze_efficiency_mathematical(
                user_profile, weather_data
            )

            # 🔮 PREDICCIÓN COMPORTAMENTAL CON CLIMA
            behavior_prediction = self._predict_mathematical_behavior_with_weather(
                user_profile, weather_data
            )

            # 💡 RECOMENDACIONES PERSONALIZADAS
            personalized_recommendations = self._generate_mathematical_recommendations(
                user_profile, weather_data, consumption_analysis, efficiency_analysis
            )

            # 📈 SCORE DE CONFIANZA MATEMÁTICO
            confidence_score = self._calculate_mathematical_confidence(
                user_profile, weather_data
            )

            ml_insights = {
                "consumption_patterns": consumption_analysis,
                "efficiency_analysis": efficiency_analysis,
                "behavior_prediction": behavior_prediction,
                "personalized_recommendations": personalized_recommendations,
                "ml_confidence_score": confidence_score,
                "analysis_timestamp": datetime.now(timezone.utc).isoformat(),
                "ml_available": False,  # Indicamos que no usamos ML
                "mathematical_analysis": True,  # Pero SÍ análisis matemático
                "weather_enhanced": True,  # Con datos meteorológicos REALES
                "weather_data": weather_data,  # Incluimos datos del clima
                "algorithm_type": "mathematical_with_weather",
            }

            logger.info("✅ Análisis matemático con datos meteorológicos completado")
            return ml_insights

        except Exception as e:
            logger.error(f"❌ Error en análisis matemático: {str(e)}")
            ...
            return {
                ...
                "ml_confidence_score": 0.3,  # Confianza baja pero funcional
                "analysis_timestamp": datetime.now(timezone.utc).isoformat(),
                "ml_available": False,
    ...
    def _analyze_consumption_with_weather(
        self, user_profile: Dict, weather_data: Dict
    ) -> Dict[str, Any]:
        """📊 Analiza consumo considerando datos meteorológicos"""
        try:
            avg_kwh = user_profile.get("avg_kwh", 250)
            peak_percent = user_profile.get("peak_percent", 50)

            # Factor climático actual
            climate_impact = weather_data.get("climate_impact", {})
            weather_multiplier = climate_impact.get("multiplier", 1.0)

            # Consumo ajustado por clima
            climate_adjusted_kwh = avg_kwh * weather_multiplier
            additional_kwh = climate_adjusted_kwh - avg_kwh

            # Análisis de patrón con clima
            if weather_multiplier > 1.15:  # +15% por clima
                pattern_type = "alto_impacto_climatico"
                climate_sensitivity = "alta"
            elif weather_multiplier > 1.05:  # +5% por clima
                pattern_type = "impacto_climatico_moderado"
                climate_sensitivity = "media"
            else:
                pattern_type = "estable_climatico"
                climate_sensitivity = "baja"

            return {
                "base_consumption_kwh": avg_kwh,
                "climate_adjusted_kwh": round(climate_adjusted_kwh, 2),
                "additional_kwh_by_weather": round(additional_kwh, 2),
                "weather_impact_percent": round((weather_multiplier - 1) * 100, 1),
                "pattern_type": pattern_type,
                "climate_sensitivity": climate_sensitivity,
                "peak_percentage": peak_percent,
                "weather_recommendations": climate_impact.get("recommendations", []),
                "temperature": weather_data.get("temperature", 20),
                "analysis_method": "mathematical_with_weather",
            }
    ...
    def _analyze_efficiency_mathematical(
        self, user_profile: Dict, weather_data: Dict
    ) -> Dict[str, Any]:
        """🏠 Análisis de eficiencia energética matemático con datos meteorológicos"""
        try:
            avg_kwh = user_profile.get("avg_kwh", 250)
            peak_percent = user_profile.get("peak_percent", 50)
            climate_impact = weather_data.get("climate_impact", {})
            ...
            # Score de eficiencia base (comparación con promedio nacional)
            national_avg = 300  # kWh promedio España
            base_efficiency = (
                max(0.3, min(1.0, national_avg / avg_kwh)) if avg_kwh > 0 else 0.5
            )

            # Ajuste por sensibilidad climática
            climate_efficiency = 1.0
            if weather_multiplier > 1.2:  # Muy sensible al clima
                climate_efficiency = 0.8  # Penalización por falta de optimización
            elif weather_multiplier > 1.1:  # Moderadamente sensible
                climate_efficiency = 0.9

            # Score final ajustado por clima
            overall_efficiency = base_efficiency * climate_efficiency

            # Análisis de equipamiento con clima
            efficiency_factors = []

            # Factor de climatización
            temperature = weather_data.get("temperature", 20)
            if temperature > 28 or temperature < 12:  # Clima extremo
                if user_profile.get("has_ac"):
                    efficiency_factors.append(
                        {
                            "factor": "climatizacion_extrema",
                            "impact": 0.15,  # 15% consumo adicional
                            "description": "Clima extremo requiere optimización de A/C",
                        }
                    )
                if user_profile.get("heating_type") == "electric":
                    efficiency_factors.append(
                        {
                            "factor": "calefaccion_electrica",
                            "impact": 0.20,  # 20% consumo adicional
                            "description": "Calefacción eléctrica en clima frío",
                        }
                    )

            # Factor de horarios punta
            if peak_percent > 60:
                efficiency_factors.append(
                    {
                        "factor": "horarios_punta",
                        "impact": 0.12,
                        "description": "Alto consumo en horas punta",
                    }
                )

            # Potencial de mejora considerando clima
            improvement_potential = (
                1 - overall_efficiency
            ) * 0.7  # Máximo 70% mejora realista

            return {
                "overall_efficiency_score": round(overall_efficiency, 3),
                "base_efficiency": round(base_efficiency, 3),
                "climate_adjustment": round(climate_efficiency, 3),
                "weather_sensitivity": "alta" if weather_multiplier > 1.15 else "media",
                "efficiency_factors": efficiency_factors,
                "improvement_potential": round(improvement_potential, 3),
                "recommended_actions": self._get_efficiency_recommendations(
                    overall_efficiency, efficiency_factors, weather_data
                ),
                "climate_optimization_score": round((2 - weather_multiplier), 3),
                "analysis_method": "mathematical_weather_enhanced",
            }
    ...
    def _get_efficiency_recommendations(
        self, efficiency_score: float, factors: List[Dict], weather_data: Dict
    ) -> List[str]:
        """💡 Genera recomendaciones específicas de eficiencia"""
        recommendations = []

        if efficiency_score < 0.6:  # Baja eficiencia
            recommendations.append(
                "Revisar aislamiento térmico para reducir impacto climático"
            )

        # Recomendaciones específicas por clima
        temperature = weather_data.get("temperature", 20)
        if temperature > 28:
            recommendations.append("Programar A/C en horarios valle (22:00-08:00)")
        elif temperature < 12:
            recommendations.append("Optimizar horarios de calefacción")

        # Recomendaciones por factores identificados
        for factor in factors:
            if factor["factor"] == "horarios_punta":
                recommendations.append("Mover consumos a horario valle")
            elif factor["factor"] == "climatizacion_extrema":
                recommendations.append("Considerar bomba de calor de alta eficiencia")

        return recommendations[:3]  # Top 3 recomendaciones

    def _predict_mathematical_behavior_with_weather(
        self, user_profile: Dict, weather_data: Dict
    ) -> Dict[str, Any]:
        """🔮 Predicción comportamental matemática con datos meteorológicos"""
        try:
            avg_kwh = user_profile.get("avg_kwh", 250)
            climate_impact = weather_data.get("climate_impact", {})
            weather_multiplier = climate_impact.get("multiplier", 1.0)

            # Predicciones estacionales ajustadas por clima actual
            current_temp = weather_data.get("temperature", 20)

            # Calcular multiplicadores estacionales más precisos
            if current_temp > 25:  # Clima caluroso - ajustar verano
                summer_multiplier = max(1.3, weather_multiplier)
                winter_multiplier = 1.15  # Invierno normal
            elif current_temp < 15:  # Clima frío - ajustar invierno
                summer_multiplier = 1.25  # Verano normal
            ...
            seasonal_predictions = {
                "verano": round(avg_kwh * summer_multiplier, 2),
                "invierno": round(avg_kwh * winter_multiplier, 2),
                "primavera": round(avg_kwh * 0.9, 2),
                "otoño": round(avg_kwh * 0.95, 2),
                "actual": round(avg_kwh * weather_multiplier, 2),
            }

            # Score de flexibilidad basado en variabilidad climática
            max_seasonal = max(seasonal_predictions.values())
            min_seasonal = min(seasonal_predictions.values())
            flexibility_score = (
                (max_seasonal - min_seasonal) / avg_kwh if avg_kwh > 0 else 0.2
            )

            return {
                "seasonal_predictions": seasonal_predictions,
                "flexibility_score": round(flexibility_score, 3),
                "weather_adaptability": "alta" if flexibility_score > 0.3 else "media",
                "optimization_willingness": (
                    "alta" if weather_multiplier > 1.1 else "media"
                ),
                "predicted_savings_potential": round(avg_kwh * 0.15, 2),
                "behavior_stability": "adaptable_clima",
                "weather_impact_analysis": climate_impact,
                "prediction_method": "mathematical_weather_enhanced",
            }

        except Exception as e:
            logger.error(f"Error en predicción comportamental: {e}")
            return {"error": str(e)}
    ...
    def _generate_mathematical_recommendations(
        ...
        """💡 Genera recomendaciones matemáticas personalizadas con clima"""
        try:
            recommendations = []

            # Obtener datos clave
            weather_multiplier = weather_data.get("climate_impact", {}).get(
                "multiplier", 1.0
            )
            temperature = weather_data.get("temperature", 20)
            additional_kwh = consumption_analysis.get("additional_kwh_by_weather", 0)

            # Recomendación 1: Optimización por clima
            if weather_multiplier > 1.15:  # +15% consumo por clima
                recommendations.append(
                    {
                        "type": "clima_optimizacion",
                        "priority": "alta",
                        "title": "Optimización por clima extremo",
                        "description": f"El clima actual (+{round((weather_multiplier-1)*100, 1)}%) está aumentando tu consumo {additional_kwh:.0f} kWh",
                        "action": (
                            "Considera tarifa con valle amplio"
                            if temperature > 25
                            else "Programa calefacción en horario valle"
                        ),
                        "potential_saving": round(
                            additional_kwh * 0.15, 2
                        ),  # 15% ahorro en exceso climático
                    }
                )

            # Recomendación 2: Horarios óptimos según clima
                ...
                recommendations.append(
                    {
                        ...
                        "potential_saving": round(additional_kwh * 0.25, 2),
                    }
                )
            elif temperature < 12:  # Frío extremo
                recommendations.append(
                    {
                        "type": "horarios_frio",
                        "priority": "alta",
                        "title": "Optimización por frío extremo",
                        "description": f"Con {temperature}°C, optimiza calefacción",
                        "action": "Calefacción programada 06-08h y 20-22h",
                        "potential_saving": round(additional_kwh * 0.20, 2),
                    }
                )

            # Recomendación 3: Tarifa óptima según patrón climático
            climate_sensitivity = consumption_analysis.get(
                "climate_sensitivity", "media"
            )
            if climate_sensitivity == "alta":
                recommendations.append(
                    {
                        "type": "tarifa_climatica",
                        "priority": "media",
                        "title": "Tarifa adaptada al clima",
                        "description": "Tu consumo es muy sensible al clima",
                        "action": "Considera tarifa discriminación horaria para aprovechar valles",
                        "potential_saving": round(
                            user_profile.get("avg_kwh", 250) * 0.12, 2
                        ),
                    }
                )
    ...
    def _calculate_mathematical_confidence(
        self, user_profile: Dict, weather_data: Dict
    ) -> float:
        """📈 Calcula score de confianza del análisis matemático"""
        try:
            confidence = 0.5  # Base: 50%

            # +20% si tenemos datos meteorológicos reales
            if weather_data.get("source") == "openweathermap_api":
                confidence += 0.2

            # +15% si tenemos datos de consumo del usuario
            if user_profile.get("avg_kwh", 0) > 0:
                confidence += 0.15

            # +10% si tenemos datos de equipamiento (A/C, calefacción)
            if user_profile.get("has_ac") or user_profile.get("heating_type"):
                confidence += 0.1

            # +5% si tenemos datos de ubicación específica
            if user_profile.get("post_code_prefix"):
                confidence += 0.05

            return min(0.95, confidence)  # Máximo 95% confianza (sin ML real)

        except Exception as e:
            logger.error(f"Error calculando confianza: {e}")
            return 0.3  # Confianza mínima funcional
    ...
    def _predict_user_behavior(self, user_profile: Dict) -> Dict[str, Any]:
        ...
        try:
            ...
            # 🌦️ INTEGRAR DATOS METEOROLÓGICOS PARA PREDICCIONES PRECISAS
            weather_data = self._get_weather_enhanced_predictions(user_profile)

            # Predicción de estacionalidad MEJORADA con datos meteorológicos
            seasonal_variation = {
                "verano": avg_kwh * weather_data.get("summer_multiplier", 1.3),
                "invierno": avg_kwh * weather_data.get("winter_multiplier", 1.2),
                "primavera": avg_kwh * weather_data.get("spring_multiplier", 0.9),
                "otoño": avg_kwh * weather_data.get("autumn_multiplier", 0.95),
            }
            ...
            return {
                ...
                "weather_impact_analysis": weather_data.get(
                    "impact_analysis", {}
    ...
    def _get_weather_data_for_user(self, user_profile: Dict) -> Dict[str, Any]:
        """🌦️ Obtiene datos meteorológicos basados en el perfil del usuario"""
        try:
            # Extraer ubicación del usuario de múltiples fuentes
            location = (
                user_profile.get("location", "")
                or user_profile.get("post_code_prefix", "") + ",ES"
                or user_profile.get("energy_profile", {}).get("location", "")
                or "Madrid,ES"  # Fallback por defecto
            )

            logger.info(f"🌍 Obteniendo datos meteorológicos para: {location}")

            # Llamada real a OpenWeather API
            weather_data = self._get_weather_data(location)

            # Enriquecer con análisis de impacto energético
            if weather_data:
                temp = weather_data.get("temperature", 20)

                # Calcular impacto climático en tiempo real
                climate_impact = self._calculate_climate_impact(temp, user_profile)

                weather_data.update(
                    {
                        "climate_impact": climate_impact,
                        "energy_adjustment": climate_impact.get("multiplier", 1.0),
                        "user_specific": True,
                    }
                )

                logger.info(
                    f"✅ Datos meteorológicos obtenidos: {temp}°C, impacto: {climate_impact.get('level', 'normal')}"
                )

            return weather_data

        except Exception as e:
            logger.warning(f"⚠️ Error obteniendo datos meteorológicos para usuario: {e}")
            ...
            return {
                ...
                "location": "Madrid,ES",
                "source": "fallback",
                "climate_impact": {"level": "normal", "multiplier": 1.0},
                "energy_adjustment": 1.0,
                "user_specific": False,
            }

    def _calculate_climate_impact(
        self, temperature: float, user_profile: Dict
    ) -> Dict[str, Any]:
        """🌡️ Calcula impacto climático personalizado según perfil del usuario"""
        try:
            # Factores base según temperatura
            if temperature > 28:
                impact_level = "muy_alto"
                base_multiplier = 1.0 + (
                    (temperature - 25) * 0.04
                )  # 4% por grado >25°C
            elif temperature > 25:
            ...
            # Ajustes según perfil del usuario
            has_ac = user_profile.get("has_ac", False) or user_profile.get(
                "energy_profile", {}
            ).get("has_ac", False)
            has_heating = (
                user_profile.get("heating_type", "") != ""
                or user_profile.get("energy_profile", {}).get("heating_type", "") != ""
            )
            home_size = user_profile.get("num_inhabitants", 2) or user_profile.get(
                "energy_profile", {}
            ).get("num_inhabitants", 2)

            # Multiplicador ajustado por equipamiento
            if temperature > 25 and has_ac:
                adjusted_multiplier = base_multiplier * 1.1  # +10% si tiene A/C
            elif temperature < 15 and has_heating:
                adjusted_multiplier = base_multiplier * 1.05  # +5% si tiene calefacción
            else:
                adjusted_multiplier = base_multiplier

            # Ajuste por tamaño del hogar
            size_factor = min(1.2, 1.0 + (home_size - 2) * 0.05)  # Max 20% aumento
            final_multiplier = adjusted_multiplier * size_factor

            # Generar recomendaciones específicas
            recommendations = []
            if temperature > 28:
                recommendations.append("Programar A/C en horario valle (22-08h)")
                recommendations.append("Temperatura óptima A/C: 24-25°C")
            elif temperature < 12:
                recommendations.append("Optimizar calefacción en valle")
                recommendations.append("Usar termostato programable")

            return {
                "level": impact_level,
                "multiplier": round(final_multiplier, 3),
                "temperature": temperature,
                "equipment_factor": has_ac or has_heating,
                "size_factor": size_factor,
                "recommendations": recommendations,
                "calculation_method": "personalizado",
            }

        except Exception as e:
            logger.error(f"Error calculando impacto climático: {e}")
            return {
                "level": "normal",
                "multiplier": 1.0,
                "error": str(e),
    ...
    def _get_weather_enhanced_predictions(self, user_profile: Dict) -> Dict:
        """🌦️ Obtiene predicciones mejoradas con datos meteorológicos reales"""
        try:
            ...
            condition = current_weather.get("weather_condition", "clear")

            # Analizar impacto en consumo energético
            impact_analysis = self._analyze_weather_impact(
                temp, humidity, condition, user_profile
            )

            # Calcular multiplicadores estacionales ajustados por clima actual
            base_consumption = user_profile.get("avg_kwh", 250)

            # Multiplicadores basados en temperatura actual y perfil de usuario
            temp_multipliers = self._calculate_temperature_multipliers(
                temp, user_profile
            )
            ...
            return {
                ...
                "location": location,
                "summer_multiplier": temp_multipliers.get("summer", 1.3),
                "winter_multiplier": temp_multipliers.get("winter", 1.2),
                "spring_multiplier": temp_multipliers.get("spring", 0.9),
                "autumn_multiplier": temp_multipliers.get("autumn", 0.95),
                "current_impact_multiplier": temp_multipliers.get("current", 1.0),
                "impact_analysis": impact_analysis,
                "weather_recommendations": self._generate_weather_recommendations(
                    current_weather, user_profile
                ),
            ...
            logger.warning(
                f"Error en predicciones meteorológicas (usando fallback): {e}"
            )
            # Fallback a multiplicadores estándar
            return {
                "current_weather": {},
                "location": "Madrid,ES",
    ...
    def _analyze_weather_impact(
        self, temp: float, humidity: float, condition: str, user_profile: Dict
    ) -> Dict:
        """🌡️ Analiza el impacto del clima en el consumo energético"""
        try:
            impact = {
                "temperature_impact": "neutral",
                "humidity_impact": "low",
                "condition_impact": "normal",
                "recommendations": [],
                "expected_change": 0,
            }
            ...
            if temp > 26:
                impact["temperature_impact"] = "high_cooling"
                impact["expected_change"] = (
                    +25 if user_profile.get("has_ac", False) else +10
                )
                impact["recommendations"].append(
                    "Alto consumo esperado por calor - considera optimizar aire acondicionado"
                )
            elif temp < 8:
                impact["temperature_impact"] = "high_heating"
                impact["expected_change"] = +20
                impact["recommendations"].append(
                    "Alto consumo esperado por frío - optimiza calefacción"
                )
            elif 18 <= temp <= 24:
                impact["temperature_impact"] = "optimal"
                impact["expected_change"] = -5
                impact["recommendations"].append(
                    "Temperatura óptima - consumo reducido esperado"
                )

            # Análisis de humedad
            if humidity > 70:
                impact["humidity_impact"] = "high"
                impact["expected_change"] += 5
                impact["recommendations"].append(
                    "Alta humedad puede incrementar uso de deshumidificador"
                )

            # Análisis de condiciones
            if condition in ["rain", "storm"]:
                impact["condition_impact"] = "indoor_increase"
                impact["expected_change"] += 8
                impact["recommendations"].append(
                    "Clima adverso - mayor tiempo interior incrementa consumo"
                )
            elif condition == "clear" and temp > 25:
                impact["condition_impact"] = "cooling_needed"
                impact["expected_change"] += 15
                impact["recommendations"].append(
                    "Sol intenso - mayor necesidad de refrigeración"
                )

            return impact

        except Exception as e:
            logger.error(f"Error analizando impacto meteorológico: {e}")
            return {"error": str(e)}

    def _calculate_temperature_multipliers(
        self, current_temp: float, user_profile: Dict
    ) -> Dict:
        """🌡️ Calcula multiplicadores de consumo basados en temperatura"""
        try:
            has_ac = user_profile.get("has_ac", False)
            has_pool = user_profile.get("has_pool", False)
            home_type = user_profile.get("home_type", "apartment")

            # Multiplicadores base por estación (ajustados por perfil)
            base_multipliers = {
                "summer": 1.3,
                "winter": 1.2,
                "spring": 0.9,
            ...
            if has_ac:
                base_multipliers["summer"] *= 1.2  # +20% más en verano con AC
            if has_pool:
                base_multipliers["summer"] *= 1.1  # +10% más por bomba piscina
            if home_type == "house":
                base_multipliers["winter"] *= 1.1  # +10% más calefacción en casa

            # Multiplicador actual basado en temperatura real
            if current_temp > 28:
                current_multiplier = 1.4 if has_ac else 1.1
            elif current_temp > 25:
                current_multiplier = 1.2 if has_ac else 1.05
            elif current_temp < 5:
                current_multiplier = 1.3
            elif current_temp < 10:
                current_multiplier = 1.15
            elif 18 <= current_temp <= 24:
                current_multiplier = 0.95  # Temperatura óptima
            else:
                current_multiplier = 1.0

            return {**base_multipliers, "current": current_multiplier}

        except Exception as e:
            logger.error(f"Error calculando multiplicadores de temperatura: {e}")
            return {
                "summer": 1.3,
                "winter": 1.2,
                "spring": 0.9,
    ...
    def _generate_weather_recommendations(
        self, weather_data: Dict, user_profile: Dict
    ) -> List[Dict]:
        """🌤️ Genera recomendaciones específicas basadas en el clima"""
        try:
            recommendations = []
            temp = weather_data.get("temperature", 20)
                ...
                recommendations.append(
                    {
                        "type": "cooling",
                        "priority": "high",
                        "title": "Optimiza refrigeración",
                        "description": f"Con {temp}°C actual, usa ventiladores antes que AC",
                        "potential_saving": "15-25%",
                    }
                )
                ...
                recommendations.append(
                    {
                        "type": "heating",
                        "priority": "high",
                        "title": "Optimiza calefacción",
                        "description": f"Con {temp}°C actual, programa calefacción eficientemente",
                        "potential_saving": "10-20%",
                    }
                )

            # Recomendaciones por condiciones
            if condition in ["clear", "sunny"] and temp > 20:
                recommendations.append(
                    {
                        "type": "solar",
                        "priority": "medium",
                        "title": "Aprovecha luz natural",
                        "description": "Día soleado - reduce iluminación artificial",
                        "potential_saving": "5-15%",
                    }
                )

            elif condition in ["rain", "cloudy"]:
                recommendations.append(
                    {
                        "type": "efficiency",
                        "priority": "medium",
                        "title": "Día interior",
                        "description": "Mayor tiempo en casa - optimiza electrodomésticos",
            ...
            logger.error(f"Error generando recomendaciones meteorológicas: {e}")
            return []
    ...
    def _get_market_data(self) -> Dict:
        ...
        try:
            ...
            SELECT 
                ...
                MIN(kwh_price_peak) as min_peak_price,
                MAX(kwh_price_peak) as max_peak_price,
                MIN(kwh_price_valley) as min_valley_price,
                MAX(kwh_price_valley) as max_valley_price
            FROM `{self.project_id}.{self.bq_dataset_id}.{self.bq_market_electricity_tariffs_table_id}`
            WHERE is_active = TRUE
            AND update_timestamp >= TIMESTAMP_SUB(CURRENT_TIMESTAMP(), INTERVAL 48 HOUR)
            AND (kwh_price_peak IS NOT NULL OR kwh_price_valley IS NOT NULL OR kwh_price_flat IS NOT NULL)
            """

            logger.info("🔍 Consultando datos ESIOS reales (48h)...")
            query_job = self.bigquery_client.query(esios_recent_query)
            results = list(query_job)

            if results and len(results) > 0:
                row = results[0]
                if row.active_tariffs and row.active_tariffs > 0:
                    # DATOS ESIOS REALES ENCONTRADOS
                    avg_peak = float(row.avg_peak_price or 0.0)
                    avg_valley = float(row.avg_valley_price or 0.0)
                    avg_flat = float(row.avg_flat_price or 0.0)

                    # Si no hay precio flat, calcularlo desde peak y valley
                    if avg_flat == 0.0 and avg_peak > 0.0 and avg_valley > 0.0:
                        avg_flat = (avg_peak + avg_valley) / 2

                    volatility = float(row.price_volatility or 0.0)
                    ...
                    logger.info(
                        f"✅ Datos ESIOS reales: Peak={avg_peak:.4f}, Valley={avg_valley:.4f}, Tarifas={row.active_tariffs}"
                    )
                    ...
                    return {
                        ...
                        "active_tariffs_count": int(row.active_tariffs),
                        "provider_count": int(row.provider_count or 0),
                        "price_range": {
                            "peak_min": round(float(row.min_peak_price or 0.0), 4),
                            "peak_max": round(float(row.max_peak_price or 0.0), 4),
                            "valley_min": round(float(row.min_valley_price or 0.0), 4),
                            "valley_max": round(float(row.max_valley_price or 0.0), 4),
                        },
                        "last_market_update": (
                            row.last_update.isoformat() if row.last_update else None
                        ),
                        "data_source": "esios_real_data_48h",
                        "timestamp": datetime.now(timezone.utc).isoformat(),
                    }

            # FASE 2: FALLBACK A DATOS HISTÓRICOS REALES (7 DÍAS)
            logger.info("🔄 Fallback a datos históricos reales (7d)...")
            historical_query = f"""
            SELECT 
                AVG(kwh_price_peak) as avg_peak_price,
                AVG(kwh_price_valley) as avg_valley_price,
            ...
            query_job = self.bigquery_client.query(historical_query)
            historical_results = list(query_job)

            if historical_results and len(historical_results) > 0:
                row = historical_results[0]
                if row.active_tariffs and row.active_tariffs > 0:
                    # DATOS HISTÓRICOS REALES ENCONTRADOS
                    avg_peak = float(row.avg_peak_price or 0.0)
                    avg_valley = float(row.avg_valley_price or 0.0)
                    avg_flat = float(row.avg_flat_price or 0.0)
                    ...
                    logger.info(
                        f"✅ Datos históricos reales: Peak={avg_peak:.4f}, Valley={avg_valley:.4f}, Tarifas={row.active_tariffs}"
                    )
                    ...
                    return {
                        ...
                        "price_trends": "historical_analysis",
                        ...
                        "data_source": "esios_historical_real_data_7d",
                        "timestamp": datetime.now(timezone.utc).isoformat(),
                    }

            # FASE 3: CONSULTAR SOLO TARIFAS PVPC ACTIVAS (ÚLTIMA OPCIÓN REAL)
            logger.info("🔄 Consultando solo tarifas PVPC reales...")
            pvpc_query = f"""
            SELECT 
                AVG(kwh_price_peak) as avg_peak_price,
                AVG(kwh_price_valley) as avg_valley_price,
            ...
            query_job = self.bigquery_client.query(pvpc_query)
            pvpc_results = list(query_job)

            if pvpc_results and len(pvpc_results) > 0:
                row = pvpc_results[0]
                if row.active_tariffs and row.active_tariffs > 0:
                    # DATOS PVPC REALES ENCONTRADOS
                    avg_peak = float(row.avg_peak_price or 0.0)
                    avg_valley = float(row.avg_valley_price or 0.0)
                    avg_flat = float(row.avg_flat_price or 0.0)
                    ...
                    logger.info(
                        f"✅ Datos PVPC reales: Peak={avg_peak:.4f}, Valley={avg_valley:.4f}"
                    )
                    ...
                    return {
                        ...
                        "price_trends": "pvpc_regulated",
                        "market_volatility": "regulated",
                        "volatility_index": 0.02,
                        "active_tariffs_count": int(row.active_tariffs),
                        "last_market_update": (
                            row.last_update.isoformat() if row.last_update else None
                        ),
                        "data_source": "pvpc_real_data_30d",
                        "timestamp": datetime.now(timezone.utc).isoformat(),
                    }

            # SI NO HAY DATOS REALES: ERROR CRÍTICO
            logger.error(
                "❌ CRÍTICO: No se encontraron datos reales de ESIOS en BigQuery"
            )
            raise Exception("No hay datos reales de ESIOS disponibles en BigQuery")

        except Exception as e:
            logger.error(f"💥 Error crítico obteniendo datos de mercado: {e}")
            raise Exception(f"Fallo crítico en obtención de datos reales: {str(e)}")
