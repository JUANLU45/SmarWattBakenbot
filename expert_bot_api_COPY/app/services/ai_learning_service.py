    ...
    def process_invoice_upload_patterns(
        ...
        try:
            ...
            learning_data = {
                ...
                "user_message": f"Subida de factura: {filename}",
                "bot_response": f"Factura procesada exitosamente",
                "conversation_id": f"invoice_{user_id}_{int(time.time())}",
            ...
            # Añadir métricas específicas de factura
            result["invoice_processing"] = {
                "filename": filename,
                "processing_success": processing_result.get("status") == "success",
                "detected_patterns": "invoice_upload_pattern",
            }

            logging.info(f"🏢 Patrones de factura procesados para usuario {user_id}")
            return result
